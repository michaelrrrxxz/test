...
$(document).ready(function() {
    ...
    //             '  h3 { font-size: 18px; margin-bottom: 10px; text-align: center; color: #333; background-color: #048e2b; color: white; padding: 10px; }' + 
    //             '  table { width: 90%; border-collapse: collapse; font-size: 12px; margin: 0 auto; }' +
    //             '  th { padding: 6px 12px; text-align: left; border-bottom: 1px solid #ddd; background-color: #048e2b; color: white; font-weight: bold; }' +
    ...
    // },
    {
        ...
        customize: function (win) {
            ...
            $(win.document.body).prepend(
                ...
                '<hr>' +
                '<h3 style="margin: 5px 0; font-size: 16pt; font-weight: bold;">No Exam Lists</h3>' +  // Increased font size for title
                '<hr>' +
                '</div>'
            );
            ...
            $(win.document.body).append(
                '<footer style="position:fixed; bottom:0; width:100%; text-align:center; font-size: 8pt; padding: 5px 0; background: #f9f9f9; border-top: 1px solid #ddd; color: #666;">' +
                'Generated on: ' + new Date().toLocaleString() + // Use .toLocaleString() to include both date and time
                '</footer>'
            );
        
            // CSS for full-width printing with professional design
            $(win.document.head).append(
                '<style>' +
                '@media print {' +
                '  body { font-family: Arial, sans-serif; color: #333; margin: 20px; }' +
                '  h3 { font-size: 14pt; margin-bottom: 10px; text-align: center; color: #333; }' +
                '  table { width: 100%; border-collapse: collapse; font-size: 9pt; margin-top: 30px; }' +
                '  th, td { padding: 6px 12px; text-align: left; border: 1px solid #ddd; font-size: 10pt; }' +
                '  th[colspan], td[colspan] { text-align: center; font-weight: bold; background-color: #f2f2f2; }' +
                ...
                '  footer { position: fixed; bottom: 0; left: 0; right: 0; text-align: center; font-size: 8pt; background: #f9f9f9; padding: 5px; color: #666; }' +
                '}' +
                '</style>'
            );
            ...
            // Hide the last column if needed (e.g., Options column)
            $(win.document.body).find('table tr').each(function () {
                $(this).find('td:last, th:last').hide();  // Adjust if you want to hide the last column
            });
        }
