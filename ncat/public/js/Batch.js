...
$(function () {
    ...
    $('#batch-table').on('click', '.status', function (e) {
        e.preventDefault();
        $('.modal-title').html('Update Batch');
        var id = $(this).data('id');
        ...
        .done(function (data, textStatus, jqXHR) {
            if (data != '') {
                if (data != '') {
                    // Only display the status field, hide the rest
                    $('#status').val(data.status);  // Set the status field
                    $('#status').removeAttr('hidden'); // Make sure the status field is visible
                    $('#statusLabel').removeAttr('hidden'); // Make sure the status label is visible
                    
                    // Hide or disable other fields
                    $('#name').attr('hidden', true); // Hide name field
                    $('#description').attr('hidden', true); // Hide description field
                    $('#duration').attr('hidden', true); // Hide duration field
                    $('#nameLabel').attr('hidden', true); // Hide name label
                    $('#descriptionLabel').attr('hidden', true); // Hide description label
                    $('#durationLabel').attr('hidden', true); // Hide duration label
                
                    // Set the ID so you know which record to update
                    $('#id').val(data.id);
