...
class EnrolledStudentController extends Controller
{
    ...
    {
        return view('Admin.EnrolledStudents.index');
    }
        ...
        public function verifyAccessKey(Request $request)
{
    ...
    // If the result exists, return with an error
    if ($resultExists) {
        return redirect()->back()->with('error', 'ID Already Exists.');
    }
    ...
    // Log the submitted and stored keys for debugging purposes
    \Log::info('Submitted Access Key: ' . $submittedKey);
    \Log::info('Active Batch Access Key: ' . $storedKey);

    // Check if the submitted access key matches the active batch's access key
    if ($submittedKey !== $storedKey) {
        return redirect()->back()->with('error', 'Invalid Access Key. Please try again.');
    }

    // Insert data into the results table if the access key is valid
    Result::create([
        'enrolled_student_id' => $student->id,
        'batch_id' => $activeBatch->id,
        'raw_score_t' => 0,
